#!/usr/bin/env python3
"""Validate or reorder bracketed AMA citations in a Markdown manuscript."""
...
def parse(path):
    ...
    if not references:
        raise ValueError("No bracketed references found")
...
def main():
    ...
    parser.add_argument("--write", action="store_true", help="Rewrite citations and references in first-use order")
    ...
    print(f"references={len(references)} citations={len(order)} sequential={sequential}")
    if undefined:
