...
PATTERNS = [
    (
        "Major",
        "结论越级",
        re.compile(r"(?:证明|证实|明确揭示|首次揭示|彻底阐明|关键机制|确切机制)"),
    ...
    (
        ...
        "空泛价值判断",
        re.compile(
            r"(?:具有(?:十分|非常|重大|重要)?(?:的)?(?:理论意义|现实意义|应用价值)|"
    ...
    (
        "Minor",
        "宏大目标",
        re.compile(r"(?:推动中医药国际化|促进中医药走向世界|助力健康中国|实现创造性转化和创新性发展)"),
...
def render_report(path, findings, phrase_counts):
    ...
    lines.extend(
        [
            ...
            "- 修改后重新执行AMA顺序、证据匹配和Reviewer 2复审。",
            "",
        ]
    )
    return "\n".join(lines)
